...
async function updateRSVPStatus(
    ...
    const response = await api.put(`/guest`, {
        ...
        rsvp_status_first: eventType === "first" ? status : undefined,
        rsvp_status_second: eventType === "second" ? status : undefined,
    });
    ...
    const rsvpMutation = useMutation({
        ...
        onSuccess: (data, variables) => {
            ...
            queryClient.invalidateQueries({
                queryKey: ["guest", guest?.invitation_code],
            });
        },
        onError: (error) => {
                ...
                <div className="grid gap-4">
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between mb-3 relative z-10">
                            ...
                            <motion.span
                                className={`px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 ${getStatusBadgeColor(guest.rsvp_status_first)}`}
                                animate={
                                    guest.rsvp_status_first ===
                            ...
                            >
                                ...
                                >
                                    {getStatusIcon(guest.rsvp_status_first)}
                                </motion.span>
                                {getEventStatusLabel(guest.rsvp_status_first)}
                            </motion.span>
                        </div>
                        {guest.rsvp_status_first ===
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between mb-3 relative z-10">
                            ...
                            <motion.span
                                className={`px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 ${getStatusBadgeColor(guest.rsvp_status_second)}`}
                                animate={
                                    guest.rsvp_status_second ===
                                        GuestEventStatus.CONFIRMED &&
                                    celebratingEvent === "second"
                                        ? {
                                              scale: [1, 1.1, 1],
                            ...
                            >
                                <motion.span
                                    ...
                                    transition={{ duration: 0.5, delay: 0.3 }}
                                >
                                    {getStatusIcon(guest.rsvp_status_second)}
                                </motion.span>
                                {getEventStatusLabel(guest.rsvp_status_second)}
                            </motion.span>
                        </div>
                        {guest.rsvp_status_second ===
                            GuestEventStatus.PENDING && (
